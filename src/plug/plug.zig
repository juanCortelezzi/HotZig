...
export fn init(gameState: *GameState) void {
    _ = gameState;
    // var player = Player{
    //     .position = .{
    //         .x = 50,
...
export fn update(gameState: *GameState) void {
    _ = gameState;
    ...
    // offset += 20;
    //
    // msg = std.fmt.bufPrintZ(&buffer, comptime "position: {}", .{@as(i32, @intFromFloat(player.position.y))}) catch unreachable;
    // c.DrawText(msg, 10, offset, 20, c.DARKGRAY);
    // offset += 20;
    //
    // msg = std.fmt.bufPrintZ(&buffer, comptime "jump_time: {}", .{@as(i32, @intFromFloat(jump_time))}) catch unreachable;
    // c.DrawText(msg, 10, offset, 20, c.DARKGRAY);
    // offset += 20;
    //
